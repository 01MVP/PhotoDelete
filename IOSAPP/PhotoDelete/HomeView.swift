...
struct HomeView: View {
    ...
    private var authorizationSection: some View {
        PhotoAuthorizationCard(
            subtitle: L10n.string("OnePhoto 需要照片库权限来整理相册。\(AppConstants.privacyShortText)"),
        ...
        VStack(spacing: 16) {
            if #available(iOS 17.0, *) {
                ContentUnavailableView(
                    ...
                    description: Text(L10n.string("当前授权范围内没有照片。您可以在系统设置里调整 OnePhoto 的照片访问范围。"))
                )
                ...
                VStack(spacing: 16) {
                    ...
                    Text(L10n.string("当前授权范围内没有照片。您可以在系统设置里调整 OnePhoto 的照片访问范围。"))
                        .font(.system(size: 15, weight: .regular))
                        .foregroundColor(PhotoDeleteStyle.secondaryText)
                        .multilineTextAlignment(.center)
                }
