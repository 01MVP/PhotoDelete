...
final class PhotoLocationTitleCacheStore {
    ...
    func titleCache() -> [String: PhotoLocationResolvedTitle] {
        titlesByGroupID
    }

    func upsert(_ title: PhotoLocationResolvedTitle, for groupID: String) {
        guard !groupID.isEmpty,
              !trimmed(title.title).isEmpty else {
            return
        }

        titlesByGroupID[groupID] = title
        save()
    }

    func merge(_ titles: [String: PhotoLocationResolvedTitle]) {
        ...
        titlesByGroupID.merge(validTitles) { _, new in new }
        save()
    }

    func prune(keeping validGroupIDs: Set<String>) {
        let pruned = titlesByGroupID.filter { validGroupIDs.contains($0.key) }
